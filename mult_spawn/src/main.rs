...
use mult_lib::proc::{PERSIST_TIMEOUT, PID};
use mult_lib::task::TaskManager;
use mult_lib::windows::proc::win_getppid;
...
fn main() -> Result<(), MultErrorTuple> {
    ...
    let task = TaskManager::get_task(&tasks, task_id.parse().unwrap())?;
    let (memory_limit, cpu_limit, _, persist) = task.options;
