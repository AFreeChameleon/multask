...
RSpec.describe "mlt restart" do
  it "Test restarting simple task" do
    ...
    original_procs = FileHelpers.read_task_processes 1, 1

    stdout, _, _ = Open3.capture3 "#{FileHelpers.get_exe} restart 1"

    changed = FileHelpers.wait_for_task_process_change 1, 10

    expect(changed).to be true

    expect(stdout).to include "[SUCCESS]"
    expect(stdout).to include "Task restarted with id 1."
  end

  it "Test restarting stopped task" do
    command = FileHelpers.is_unix ? "sleep 5" : "powershell -C Start-Sleep -Seconds 5"
    ...
    running = ProcessHelpers.wait_for_task_running 1, 20
    expect(running).to be true
  end

  it "Test restarting task with all flags enabled" do
    _, _, _ = Open3.capture3 "#{FileHelpers.get_exe} c echo hi --no-run"
    sleep 0.1

    stdout, _, _ = Open3.capture3 "#{FileHelpers.get_exe} restart 1 -c 20 -m 20M -ip -s deep"
    sleep 0.1
  ...
  it "Test restarting task updating environment variables" do
    _, _, _ = Open3.capture3 "#{FileHelpers.get_exe} c echo hi --no-run"
    sleep 0.1

    ENV["NEW_ENV"] = "NEW_ENV_VALUE"

    _, _, _ = Open3.capture3 "#{FileHelpers.get_exe} r 1 -e"
    sleep 0.1
  ...
  it "Test restarting task that doesn't exist" do
    _, stderr, _ = Open3.capture3 "#{FileHelpers.get_exe} restart 1"
    sleep 0.1
