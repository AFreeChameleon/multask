...
use windows_sys::Win32::{Foundation::CloseHandle, System::{JobObjects::{AssignProcessToJobObject, CreateJobObjectA, JobObjectCpuRateControlInformation, JobObjectExtendedLimitInformation, JobObjectNotificationLimitInformation, OpenJobObjectA, SetInformationJobObject, JOBOBJECT_CPU_RATE_CONTROL_INFORMATION, JOBOBJECT_CPU_RATE_CONTROL_INFORMATION_0, JOBOBJECT_EXTENDED_LIMIT_INFORMATION, JOBOBJECT_NOTIFICATION_LIMIT_INFORMATION, JOB_OBJECT_CPU_RATE_CONTROL_ENABLE, JOB_OBJECT_CPU_RATE_CONTROL_HARD_CAP, JOB_OBJECT_CPU_RATE_CONTROL_MIN_MAX_RATE}, Threading::{CreateProcessA, WaitForSingleObject, CREATE_NO_WINDOW, INFINITE, PROCESS_INFORMATION, STARTUPINFOA}}};
use crate::{command::MemStats, error::{print_error, MultError, MultErrorTuple}, task::Files};

pub fn run_daemon(files: Files, command: String, flags: &MemStats, task_id: u32) -> Result<(), MultErrorTuple> {
            ...
            if CreateProcessA(
                ...
                if flags.cpu_limit != -1 {
                    let job_limit_info = JOBOBJECT_CPU_RATE_CONTROL_INFORMATION {
                        ControlFlags: JOB_OBJECT_CPU_RATE_CONTROL_HARD_CAP | JOB_OBJECT_CPU_RATE_CONTROL_ENABLE,
